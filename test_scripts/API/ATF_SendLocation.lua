...
function Test:SendLocation_AllowedParamsBase4_ParamsNotPresentedInPolicies_NotAnswerForConsentGroup1()
	...
	:ValidIf(function(_,data)
		if data.params.address or data.params.addressLines or data.params.deliveryMode or data.params.locationImage or data.params.phoneNumber or data.params.locationName or data.params.locationDescription or data.params.timeStamp then
			commonFunctions:userPrint(31,"Navigation.SendLocation contain some parameters in request when should be omitted")
			return false
		else
			return true
...
function Test:SendLocation_ParamsInBase4_ParamInGroup1_User_Answer_NO()
	...
	-- mobile side: expect SendLocation response
	EXPECT_RESPONSE(cid, { success = true, resultCode = "SUCCESS", info = "'address', 'deliveryMode' are disallowed by user"})			
end
