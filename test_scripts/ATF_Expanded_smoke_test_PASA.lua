...
---------------------------------------------------------------------------------------------
-- Update SmokeTest_pt.json: app id to file
local pt_file = "./files/SmokeTest_pt.json"

f = assert(io.open(pt_file, "r"))

fileContent = f:read("*all")

local FindAppIdLabel = fileContent:match('"app_id"')
local FindAppId = fileContent:match('"' .. tostring(config.application1.registerAppInterfaceParams.appID .. '"'))
if FindAppIdLabel then
  fileContentUpdated  =  string.gsub(fileContent, "app_id", tostring(config.application1.registerAppInterfaceParams.appID))
  f = assert(io.open(pt_file, "w"))
  f:write(fileContentUpdated)
else
  if FindAppId then
    print ( " \27[33m app_id value is already updated to actual app id \27[0m " )
  else
    print ( " \27[33m Error with updating SmokeTest_pt.json: missed app_id label and actual value of app id. Please check SmokeTest_pt.json. \n Script can be executed with unexpected result. \27[0m " )
  end
end
...
---------------------------------------------------------------------------------------------
	...
	-- Description: Update of Polcy table
	  	function Test:PolicyUpdate()
	    	UpdatePolicy(self, "files/SmokeTest_pt.json")
	  	end
	--End Precondition.3
----------------------------------------------------------------------------------------- 
-----------------------------------------------------------------------------------------
